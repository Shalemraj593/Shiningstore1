...
function getBadge(id) {
    const badges = ['New', 'Sale', 'Hot', 'Best Seller', null, null, null, null];
...
async function init() {
    ...
    // Inject mock sizes and colors for advanced filtering
    const colors = ['black', 'white', 'red', 'blue', 'beige'];
    const sizes = ['XS', 'S', 'M', 'L', 'XL'];
    products.forEach(p => {
        if (!p.colors) p.colors = [colors[Math.floor(Math.random() * colors.length)], colors[Math.floor(Math.random() * colors.length)]];
        if (!p.sizes) p.sizes = sizes.slice(Math.floor(Math.random() * 3)); // Random subset of sizes
    });
    ...
    productGrids.forEach(grid => {
        ...
        limitedItems.forEach((product, index) => {
            ...
            if (product.comparePrice) {
                ...
                priceHTML = `<div class="product-price-strike">₹${cINR}</div><div class="product-price-offer">Offer Price ₹${pINR}</div>`;
            } else {
                const pINR = Math.round(product.price * EXCHANGE_RATE_INR);
                const offerINR = Math.round(pINR * 0.7);
                priceHTML = `<div class="product-price-strike">₹${pINR}</div><div class="product-price-offer">Offer Price ₹${offerINR}</div>`;
            }
    ...
    function initAdvancedFilters() {
        ...
        function renderFiltered() {
            const filtered = products.filter(p => {
                ...
                const passCat = activeCats.length === 0 || activeCats.includes(p.category);
                ...
                return passPrice && passCat && passSize && passColor;
            });
        ...
        function updateActiveTags() {
            ...
            activeCats.forEach(cat => {
                activeFiltersContainer.innerHTML += `<div class="active-tag">${cat} <i class="fa-solid fa-xmark" onclick="removeFilter('cat', '${cat}')"></i></div>`;
            });
        }
...
function initChatbot() {
            ...
            <div class="chatbot-window" id="chatbotWindow">
                ...
                <div class="chatbot-messages" id="chatbotMessages">
                    <div class="chat-msg bot-msg">
                        Hi there! 👋 Welcome to Shining Store. How can I help you today?
                    </div>
                </div>
    ...
    function handleSend() {
        ...
        setTimeout(() => {
            ...
            let reply = "I'm here to help with your premium shopping experience.";
            const t = text.toLowerCase();
            if (t.includes('order') || t.includes('track')) {
                reply = "You can track your order by logging into your Account Dashboard, or using our Order Tracking page!";
            } else if (t.includes('return') || t.includes('refund')) {
                reply = "We offer a 30-day hassle-free return policy. Shall I start a return process for you?";
            } else if (t.includes('shipping') || t.includes('delivery')) {
                reply = "Good news! We offer Free Shipping on all orders above $50. Standard delivery takes 3-5 days.";
